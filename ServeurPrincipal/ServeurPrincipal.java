...
public class ServeurPrincipal 
{
    ...
    private static class ClientHandler implements Runnable 
    {
        ...
        private static void divideAndSendFile(File file, List<Socket> availableServers) throws IOException 
        {
            ...
            File backupDir = new File(serverPrincipalDirectory + "/Backup");
            if (!backupDir.exists()) 
            {
                if (backupDir.mkdir()) {
                    System.out.println("Répertoire Backup créé.");
                } else {
                    System.err.println("Échec de la création du répertoire Backup.");
                    return;
                }
            }
            ...
            for (int i = 0; i < nbServers; i++) 
            {
                ...
                // Sauvegarder cette partie pour la redondance
                File backupFile = new File(backupDir, partFile.getName());
                Files.copy(partFile.toPath(), backupFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
        ...
        private static void handleGet(DataInputStream clientInputStream, DataOutputStream clientOutputStream) throws IOException 
        {
            ...
            ByteArrayOutputStream completeFileData = new ByteArrayOutputStream();
            ...
            for (int i = 1; i <= secondaryServerAddresses.size(); i++) 
            {
                ...
                try (Socket secondarySocket = new Socket(serverAddress, serverPort);
                    ...
                    String partName = "part_" + i + "_received_" + fileName;
                    ...
                    if (fileSize == -1) 
                    {
                        ...
                        // Si le serveur secondaire est injoignable, essayer de récupérer la partie depuis une sauvegarde
                        File backupFile = new File(serverPrincipalDirectory+"/Backup/part_" + i + "_" + fileName);
                        if (backupFile.exists()) 
                        {
                            System.out.println("Serveur secondaire injoignable, récupération depuis le backup : " + backupFile.getName());
                            try (FileInputStream backupInputStream = new FileInputStream(backupFile)) 
                            {
                                byte[] buffer = new byte[1024];
                                int bytesRead;
                                while ((bytesRead = backupInputStream.read(buffer)) != -1) {
                                    completeFileData.write(buffer, 0, bytesRead);
                                }
                            }
                            continue; // Continuer avec la récupération des autres parties
                        } else {
                            continue; // Passer à la partie suivante si aucune sauvegarde n'existe
                        }
                    }
                ...
                catch (IOException e) 
                {
                    ...
                    // Essayer de récupérer la partie depuis la sauvegarde
                    File backupFile = new File(serverPrincipalDirectory+"/Backup/part_" + i + "_received_" + fileName);
                    if (backupFile.exists()) 
                    {
                        System.out.println("Récupération de la partie " + i + " depuis la sauvegarde.");
                        try (FileInputStream backupInputStream = new FileInputStream(backupFile)) 
                        {
                            byte[] buffer = new byte[1024];
                            int bytesRead;
            ...
            // Vérification et envoi des données du fichier complet
            ...
            File tempDirectory = new File(serverPrincipalDirectory+"/Backup"); // Répertoire temporaire de récupération, à ajuster
            if (tempDirectory.exists()) 
            {
                deleteBackup(tempDirectory);
            }
        }


        private static void deleteBackup(File directory) 
        {
            if (directory.isDirectory()) {
                for (File file : Objects.requireNonNull(directory.listFiles())) {
                    deleteBackup(file);  // Récursion pour supprimer les fichiers dans le répertoire
                }
            }
            // Supprimer le fichier ou répertoire actuel
            if (directory.delete()) {
                // System.out.println("Fichier ou répertoire supprimé : " + directory.getAbsolutePath());
            } else {
                System.err.println("Erreur lors de la suppression du fichier ou répertoire : " + directory.getAbsolutePath());
            }
        }
